...
import ThemeText, {TitleText, BodyText} from '../ThemeText';
  ...
  return (
    <Animated.View
      ...
      <View style={tw`mb-6 px-4`}>
        <TitleText style={tw`text-3xl text-gray-800 mb-1`}>Shopping</TitleText>
        <BodyText style={tw`text-gray-800`}>Perfect Choice!</BodyText>
      </View>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false}>
        ...
        {categories.map((item, index) => (
          <Buttons.HeaderNavigationButton
            key={index}
            text={item.title}
            onPress={() => dispatch(categoryHandler(item.id))}
